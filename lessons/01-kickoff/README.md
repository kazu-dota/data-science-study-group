...
- `CORE`：入力データ（特徴量）から、活性あり／なしの予測とその確率を出す
- `DEEP DIVE`：予測確率のばらつきと、どの入力が効いたか（特徴量重要度）を読み、1件の結果だけで判断しない
- `SELF-STUDY`：決定木の深さ2・4・8を同じ検証データで比較する

見慣れない言葉があっても大丈夫です。意味はNotebook内でそのつど説明します。
