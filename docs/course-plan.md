...
同期回を長くせず内容を深めるため、各Notebookを次の3層に分けます。
...
全員が全セルを終えることは目標にしません。ただし、深掘りを選んだ人も「予想・変更点・出力・解釈」の4点を残し、
...
| 回 | 同期回の本線 | 一段深い実験・考察 |
|---:|---|---|
| 1 | 1試料のクラスと確率を予測 | 予測確率の分布、特徴量重要度、木の深さによる変化 |
| 2 | Pythonの基本構文を読み変更 | 型ヒント、入力検査、境界値、例外の読み分け |
| 3 | 表の健康診断と群別集計 | 欠損率・一意数の品質表、触媒×溶媒の二軸集計 |
| 4 | 分布・欠損・外れ値を可視化 | 相関ヒートマップ、群別欠損率、相関と因果の区別 |
| 5 | 予測問題とDummyを定義 | 問題設定キャンバス、誤りの種類と業務コスト |
| 6 | 過学習とリークを体験 | `GroupKFold`による系列分割、平均と標準偏差 |
| 7 | 回帰モデルと指標を比較 | 系列別MAEと平均残差、系統的な過大・過小予測 |
| 8 | 混同行列と閾値を比較 | Precision–Recall曲線上の閾値探索と過適合への注意 |
| 9 | 前処理をPipeline化 | One-Hot後の特徴量名・列数・変換値を確認 |
| 10 | 5モデルを同条件で比較 | 5-fold交差検証の平均・標準偏差・最低F1 |
| 11 | 仮説特徴量を追加 | 特徴量を1つずつ足すアブレーション比較 |
| 12 | 1要素ずつ改善を記録 | holdout上のpermutation importanceと実験ログ保存 |
| 13 | ベースライン提出を作成 | ID順・列・行数・値域まで検査する提出バリデータ |
| 14 | 分担して改善案を比較 | 化合物系列ごとの件数・誤分類数・誤分類率 |
| 15 | 結果と次の一歩を共有 | 用途・評価・限界・禁止条件を含むモデルカード |

この表の右列は全員必修ではありません。講師は参加者の反応を見て1つだけ同期回で扱い、残りを任意自習に回せます。
