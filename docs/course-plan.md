...
- 本線の`基本`でも、関数にまとめる・検証方法を工夫する・何もしないモデルと比べる、といった実践的な内容まで扱う。`発展（任意）`ではさらに、評価の厳密さとモデルの多様さを深める（用語は各回で説明します）
...
この勉強会は**全5回**です。各回は3つのパート（第5回のみ4つ）に分かれます。
...
「通し番号」は全パートを上から順に数えた番号です。1〜3が第1回、4〜6が第2回という順で対応します。
...
右列は全員必修ではありません。講師は参加者の反応を見て1〜2個だけ同期回で扱い、残りを任意自習へ回します。`補足`は本線に近く全員向け、`発展（任意）`は経験者や自習向けです。
