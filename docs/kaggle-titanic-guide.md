...
この勉強会で学んだ「問題確認→ローカル検証→提出CSV作成→提出」の流れを、実際のKaggle過去コンペで1回経験する**任意教材**です。順位を競うことは目的にしません。本編（全5回）には含まれないため、興味がある人だけ取り組みます。
...
[任意実践Notebook](../lessons/05-advanced-and-operate/titanic_optional.ipynb)を上から順に実行します。データがまだない場合は、置き場所を表示して安全に停止します。
...
Kaggleアカウントを作れない、社内ネットワークからアクセスできない、ルールへ同意できない場合は、この任意教材はスキップして構いません。本編（全5回）の学習目標には影響しません。
