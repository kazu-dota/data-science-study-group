...
420件の架空実験を収録しています。第1回のパート3（pandas）から第4回まで同じデータを繰り返し使い、データの見方からモデル改善まで段階的に学びます。
...
第5回のパート1・2（模擬Kaggle）で使います。

- `train.csv`：説明変数と`active`
- `test.csv`：説明変数のみ
- `sample_submission.csv`：提出形式の見本
- `instructor_answers.csv`：ローカル採点用の答え。第5回パート2の採点まで開かない運用を推奨
